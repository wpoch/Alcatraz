...
namespace Alcatraz.Core.Helpers
{
    public static class TaskAsyncHelper
    {
        ...
        public static TTask Catch<TTask>(this TTask task) where TTask : Task
        {
            if (task != null && task.Status != TaskStatus.RanToCompletion)
            {
                task.ContinueWith(innerTask =>
                                      {
                                          AggregateException ex = innerTask.Exception;
                                          // observe Exception
        ...
        {
            Task<T> innerTask = (task.Status == TaskStatus.RanToCompletion) ? task.Result : null;
            return innerTask ?? task.Unwrap();
        }
        ...
        public static Task AllSucceeded(this Task[] tasks, Action<Task[]> continuation)
        {
            return Task.Factory.ContinueWhenAll(tasks, _ =>
                                                           {
                                                               Task cancelledTask =
                                                                   tasks.FirstOrDefault(task => task.IsCanceled);
                                                               if (cancelledTask != null)
                                                                   throw new TaskCanceledException();

                                                               List<Exception> allExceptions =
                                                                   tasks.Where(task => task.IsFaulted).SelectMany(
                                                                       task => task.Exception.InnerExceptions).ToList();
        ...
        {
            TaskContinueWithMethod continueWith = (from m in taskType.GetMethods()
                                                   let methodParameters = m.GetParameters()
                                                   where
                                                       m.Name.Equals("ContinueWith", StringComparison.OrdinalIgnoreCase) &&
        ...
        public static Task Then(this Task[] tasks, Action successor)
        {
            ...
            Task.Factory.ContinueWhenAll(tasks, completedTasks =>
                                                    {
                                                        Task faulted = completedTasks.FirstOrDefault(t => t.IsFaulted);
                                                        if (faulted != null)
                                                        {
                                                            tcs.SetException(faulted.Exception);
                                                            return;
                                                        }
                                                        Task cancelled = completedTasks.FirstOrDefault(t => t.IsCanceled);
                                                        if (cancelled != null)
                                                        {
                                                            tcs.SetCanceled();
                                                            return;
        ...
        public static Task<TResult> Then<TResult, T1>(this Task<TResult> task,
                                                      Func<Task<TResult>, T1, Task<TResult>> successor, T1 arg1)
        {
            switch (task.Status)
            {
                case TaskStatus.Faulted:
        ...
        #region Nested type: GenericDelegates
        ...
        #region Nested type: TaskContinueWithMethod
        ...
        #region Nested type: TaskRunners
